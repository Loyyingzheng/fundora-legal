...
assert(app.includes('headers = await createAuthenticatedApiHeaders(path, options, body, true)'), 'A permitted retry must rebuild headers with a fresh nonce.');
...
assert(html.includes('account-security-ux-v5-nonce-retry'), 'Nonce retry fix must be cache-busted.');
